...
def subject_data(sub_dir):
    ...
    # 归一化 to -1 1
    # b5000 = normalization(b5000, 300)
    # b10000 = normalization(b10000, 200)

    # 140 140 96 256 to 3 140 140 96 256
    # low_b_DWI = np.stack([b1000, b3000, b5000], axis=0)

    low_b_DWI = np.expand_dims(b5000, axis=0)



    # plt.imshow(low_b_DWI[0, :, :, 48, 0], cmap='gray')
    # plt.show()
    # plt.imshow(low_b_DWI[1, :, :, 48, 0], cmap='gray')
    # plt.show()
    # plt.imshow(low_b_DWI[2, :, :, 48, 0], cmap='gray')
    # plt.show()
    # to 3 140 140 96*256
...
sub_list = sorted(os.listdir(dataset_path))[:5]
print('training sub list : ', sub_list)
count = 0
for file in sub_list:
    sub_folder = join(dataset_path, file)

    low_b_DWI, high_b_DWI = subject_data(sub_folder)
...
low_b_DWI_file = h5py.File('/data/wtl/SR_Data/low_b_DWI.h5', 'w')
high_b_DWI_file = h5py.File('/data/wtl/SR_Data/high_b_DWI.h5', 'w')
